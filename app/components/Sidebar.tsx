  ...
  return <aside className='sidebar'>
    ...
    <nav className='sidebar-nav'>
        ...
        {navItems.map(({ url, name, icon }) => {
          ...
          return (
            <Link key={name} href={linkUrl} className='lg:w-full'>
              <li className={cn('sidebar-nav-item', pathname === linkUrl && 'shad-active')}>
                <Image src={icon} alt={name} width={24} height={24} className={cn('nav-icon', pathname === linkUrl && 'nav-icon-active')} />
      ...
      <div className='sidebar-user-info'>
        <Image src={avatar && avatar.trim() !== '' ? avatar : avatarPlaceholderUrl} alt='avatar' width={44} height={44} className='sidebar-user-avatar'/>
